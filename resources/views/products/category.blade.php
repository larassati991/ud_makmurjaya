...
@section('title', $category->name . ' - UD MAKMUR JAYA DAGING')
...
<section class="relative h-80 md:h-96 flex items-center">
    ...
    <div class="container mx-auto px-4 relative z-10">
        <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold text-white">{{ $category->name }}</h1>
    </div>
</section>
...
<section class="bg-gray-100 py-4">
    <div class="container mx-auto px-4">
        <div class="flex items-center text-sm text-gray-600">
            ...
            <a href="{{ route('products.index') }}" class="hover:text-red-600 transition">Katalog Produk</a>
            <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
    ...
    <div class="container mx-auto px-4">
        ...
        @if($products->count() > 0)
            ...
            <div class="mb-8">
                <p class="text-gray-600 text-lg">
                    Menampilkan <strong class="text-red-600">{{ $products->count() }}</strong> produk {{ $category->name }}
                </p>
            </div>
                    ...
                    <div class="relative overflow-hidden h-64">
                        ...
                        @else
                            ...
                            <div class="w-full h-full bg-gradient-to-br from-red-400 to-red-600 flex items-center justify-center">
                                <span class="text-white text-6xl font-bold">{{ substr($product->name, 0, 1) }}</span>
                            </div>
                        @endif
                    ...
                    <div class="p-6">
                        ...
                        @if($product->weight)
                            <p class="text-sm text-gray-500 mb-4 flex items-center">
                                ...
                                <span class="font-semibold">Berat:</span> {{ number_format($product->weight, 0, ',', '.') }} kg
                            </p>
                        @endif
                        
                        <!-- Order Button -->
                                <a href="https://wa.me/{{ preg_replace('/\\D+/', '', App\Models\Setting::get('whatsapp_number', '+62 852-2546-1504')) }}?text=Halo, saya tertarik dengan produk {{ $product->name }}" 
                           target="_blank"
        ...
        @else
            ...
            <div class="text-center py-20">
                ...
                <p class="text-gray-500 mb-6">Saat ini belum ada produk {{ $category->name }}</p>
                <a href="{{ route('products.index') }}" class="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-8 rounded-lg transition">
                    Lihat Kategori Lain
                </a>
            </div>
        @endif
    ...
    <div class="container mx-auto px-4">
        <div class="bg-gradient-to-r from-red-600 to-red-700 rounded-2xl p-8 md:p-12 text-center text-white">
            ...
            <p class="text-xl mb-8 text-white/90">Tim kami siap membantu Anda menemukan produk yang sesuai dengan kebutuhan bisnis Anda</p>
                <a href="https://wa.me/{{ preg_replace('/\\D+/', '', App\Models\Setting::get('whatsapp_number', '+62 852-2546-1504')) }}?text=Halo, saya butuh bantuan memilih produk {{ $category->name }}" 
               target="_blank"
